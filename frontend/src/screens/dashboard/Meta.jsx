...
const VIEWS = [
  { id: "paid", name: "Betaald-overzicht", audience: "Directie" },
  { id: "campaigns", name: "Campagnes", audience: "Marketeer" },
  { id: "facebook", name: "Organisch — Facebook", audience: "Social" },
  { id: "instagram", name: "Organisch — Instagram", audience: "Social" },
  { id: "vs", name: "Betaald vs. organisch", audience: "Directie" },
];

export default function Meta() {
  const { orgId } = useActiveOrg();
  const { start, end, compare, label } = useDateRange();
  const [view, setView] = useState(() => localStorage.getItem("kompas-meta-view") || "paid");
  const pickView = (id) => { setView(id); localStorage.setItem("kompas-meta-view", id); };
  ...
  if (!adAccounts?.length && !pages?.length)
    return (
      <div>
        <Header label={label} />
        <div className="card" style={{ padding: 28, color: "var(--c-muted)" }}>Geen Meta-advertentieaccounts of pagina's gevonden voor deze koppeling.</div>
      </div>
    );

  const k = ads?.kpis;
  const paidView = view === "paid" || view === "campaigns" || view === "vs";
  const orgView = view === "facebook" || view === "instagram" || view === "vs";
    ...
    <div>
      ...
      {view === "facebook" && (
        <SectionCard title="Facebook-pagina">
          {!pageId ? <Empty>Geen Facebook-pagina gekoppeld.</Empty> : (organic?.facebook && Object.keys(organic.facebook).length) ? (
            <>
              <KpiRow items={[
      ...
      {/* ---- Organisch Instagram ---- */}
      {view === "instagram" && (
        <SectionCard title={"Instagram" + (organic?.instagram?.username ? " · @" + organic.instagram.username : "")}>
          {ig == null ? <Empty>geen gekoppeld Instagram-account.</Empty> : organic?.instagram ? (
            <>
              <KpiRow items={[
      ...
      {view === "vs" && (
        <SectionCard title="betaald vs. organisch">
          <div style={{ fontSize: 12.5, color: "var(--c-muted)", marginBottom: 14 }}>
            Wat advertenties toevoegen bovenop je organische bereik en betrokkenheid.
          </div>
          <KpiRow items={[
            ["Betaald bereik", num(k?.reach)],
            ["Organisch bereik", num((organic?.facebook?.reach || 0) + (organic?.instagram?.reach || 0))],
            ["Betaalde resultaten", num((ads?.results || []).reduce((a, r) => a + (r.count || 0), 0))],
